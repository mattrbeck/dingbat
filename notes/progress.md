...
Write-ups of individual pieces of work that are worth keeping for their method:
the measurement, what was tried and refused, and the traps that cost time. They
are dated and not maintained — check them against the code before acting.

This is **not** a status document. For what is implemented, see
`docs/features.md`; for what is open, `notes/todo.md`; for the port's history,
`git log`. An earlier revision of this file carried a per-subsystem status table
that had drifted badly out of date (it described serial I/O as stubbed and most
HLE SWIs as no-ops, long after both landed), plus a phase-by-phase account of the
original Crystal→Nim port that git already records.

...
export const loadApp = async ({ localStorageSeed = {}, confirmResult = true,
  ...
  const sandbox = {
    ...
    navigator: {
      ...
      storage: {
        estimate: async () => ({
          usage: 12345,
          quota: state.storageQuota ?? 4 * 1024 * 1024 * 1024,
        }),
  ...
  const api = vm.runInContext(`({
    ...
    addRecentRom, bumpRecentIndex, touchRecent, romBudget,
    ROM_BUDGET_SHARE, ROM_BUDGET_MIN, ROM_BUDGET_MAX,
    evictLocalRom, enforceRomBudget, noteRomSize, updateStorageInfo,
    ...
    get romsSort() { return romsSort; },
    get libFilter() { return libFilter; },
    maybeOfferThumbnails, runThumbnailBatch, cancelThumbnailRun, thumbsCandidates,
    offerThumbnailsAfterBoot,
    openThumbsRun,
    THUMBS_OFFER_KEY, THUMBS_RESUME_FRAMES, THUMBS_BOOT_FRAMES,
    ...
    driveLinked, driveEnrolled, adoptDriveAccount, gdriveSignOut, gdriveSignOutEverywhere, gdriveFetchEmail,
    gdriveAcquireToken, ensureDriveSignedIn, gdriveConnect,
    driveRefreshSilently, driveCodeGrant, probeDriveBroker, driveWantsUpgrade,
    driveBrokerBase,
    get driveBrokerRetryAt() { return driveBrokerRetryAt; },
    set driveBrokerRetryAt(v) { driveBrokerRetryAt = v; },
    get driveBrokerOk() { return driveBrokerOk; },
    set driveBrokerOk(v) { driveBrokerOk = v; },
    get driveBrokerProbedAt() { return driveBrokerProbedAt; },
    set driveBrokerProbedAt(v) { driveBrokerProbedAt = v; },
    ...
    get rollbackMode() { return rollbackMode; },
    set rollbackMode(v) { rollbackMode = v; },

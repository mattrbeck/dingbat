#!/bin/bash
# Assemble flashcart-kit/ — everything to load on a GB flashcart for a
# hardware-validation session (docs/flashcart-runbook.md says what to do with
# each ROM).
#
# Sources: the repo's probe ROMs, the game-boy-test-roms bundle in
# $DINGBAT_ROM_CACHE (default /tmp/dingbat-test-roms; run ./dingbat_test_runner
# once to populate it), nitro2k01's windesync ROM (downloaded), and daid's
# ppu_scanline_bgp from a GBEmulatorShootout clone in ~/code if present.
set -euo pipefail
cd "$(dirname "$0")/.."
...
# 1 — the gbedge probe ROM
cp tests/roms/gbedge.gb "$KIT/1-gbedge/"

# 2 — nitro2k01's window-desync ROM (hwprobe row 19)
...
# 3 — window-glitch neighbours: two mealybug window ROMs and strikethrough
#     (hwprobe row 18)
...
# 4 — timer: CGB TAC-disable arbitration
...
# 6 — the CGB obj-fetch-while-disabled phase
...
# 8 — cgb-acid-hell, daid's band-edge ROM, and the probe rig. probe (c) is a
#     DMG cart on purpose (its BGP ruler is inert in native-CGB mode); probe
#     (d) is CGB-flagged, with a _compat build as the control.
...
# probe_cart: run this first; it checks the flashcart returns the ROM's real
# bytes (three access orders + an address-line hammer).
mkdir -p "$KIT"/0-cart-check
cp tools/gbprobe/probe_cart.gb "$KIT/0-cart-check/"

# probe (d): tile-select latency. Read by eye (sixteen bands) or with
# read_probe_d.py; the question is docs/hwprobe-questions.md.
...
# (up/down). With the object OFF it is probe (d).
...
# 7 — APU: CH3 buffer-on-DAC-off + sweep restarts
...
# 9 — g1: the halt-wake PPU phase (docs/hwprobe-questions.md). Two fixed
# builds rather than the paged ROM: the question is a 4-dot phase and a wrong
# d-pad press would silently answer a different one. Each ROM prints its own

...
| A8 | OAM_Corruption_Bug: "Any memory access instruction, if it accesses OAM" in mode 2 corrupts | only the 16-bit IDU family (inc/dec rr, ld [hl±], push/pop, dispatch) reaches `oam_bug_if`; plain loads go through `cpu_oam_open` | blargg `oam_bug` is green either way (its causes ROMs never test plain accesses). Assumed; no ROM pins this. Deferred: wiring it must not double-fire the IDU sites |
| A9 | Audio_details: envelope timer gets +1 when a trigger lands just before an envelope step | `init_volume_envelope` sets `timer = period` unconditionally (the length analogue is modelled) | no SameSuite row pins it; Assumed |
| A10 | Audio_Registers NR10: pace 0→nonzero write reloads the sweep timer | NR10 write stores fields only; the "instantly disabled" half is modelled | no SameSuite row pins it; Assumed |

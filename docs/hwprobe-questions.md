...
| Timer prescaler boot phase (hw staircase `07 07 07 06 06 06` vs dingbat `06 07 07 06 06 07`); is the prescaler one free-running divider shared across timers, does enable reset it; PSG length expiry 14 % early (hw 2E1E vs 283A) and the length-63 trigger-adjacent clock; does SOUNDCNT_X master-off reset the sequencer | TIMERS, PSGSTAT, SWEEPQ | **run 2026-09-04** — p43 TIMPHASE ratifies the shared free-running divider (no change); p44 PSGPHASE: the first trigger after a SOUNDCNT_X power-on dies for any length, ch2 two stores later lives. **p51 PSGFIRST run 2026-09-16**: ch1 only (ch2/3/4 live), and not a first-trigger effect (ch1's second trigger dies too, a third lives); needs length enable set and NR10 = 0. Still open: never-enables vs active bit rises late — both pages poll only for the fall. **p52 PSGWHY run 2026-09-18** over the link rig, four times. "Never-enables vs rises late" was a false dichotomy: the bit is already set on the first poll after the store and has fallen by the next — the channel enables and is then killed. The f + f candidate holds on the stable rows (f = 0x400 dies, 0x3FF lives, 0 lives, the 0x800 boundary). Both emulators have the mechanism backwards where a sweep overflow is unambiguous (f = 0x7FF shift 1): they refuse the trigger and never enable, hardware accepts then kills. **Caveat: 6 of 15 rows vary run to run** — the payload polls from IWRAM faster than this cartridge-resident loop was calibrated for, so only the repeatable rows are reported; settle the rest from a cart or with a rate-matched delay. Nothing changed (docs/playtest-bugs.md §12) |
...
| When in the scanline an H-blank DMA is granted the bus: the end-of-draw edge at cycle 960 (dingbat) or the latched DISPSTAT condition that also raises the H-blank IRQ, 960+46+6 = 1012. The ~52-cycle difference is the whole of the mGBA suite's last red row, Misc "DMA Prefetch Break" (dingbat `0x100026D4` vs hw `0x10002A94`); grant at 1012 closes it and moves no other row of the 1219-row runner, but nothing else in either ROM constrains the constant. p46 DMATIME `+24` reads 37 on both and cannot discriminate — both its ends are ROM-resident polls | HDMAPHASE | **run 2026-09-16** — p50: the H-blank DMA writes **48 cycles later** than dingbat on two lines with zero spread, flag and V-blank DMA stamps exact: the grant follows the latched flag (cycle 1008–1009 = flag + 2–3), not the cycle-960 edge and not the suite-fit 1012. The write also moves +5/+2 with the CPU on ROM bus cycles, so the grant waits on the bus cycle in flight; immediate DMA writes 1 cycle earlier than dingbat (docs/hwprobe-results-agb.md, session 6). Request at flag + 2 reproduces p50's H-blank stamps exactly (branch `dma-flag-grant`). **p53 HDMASWEEP run 2026-09-18** over the link rig, six times: neither prediction. Not a sawtooth and not flat — k = 4,5,7–13 sit at 227 with no deferral, k = 6 loses a stable **11 cycles** in all six runs, and k = 0–3 tip between the two run to run. So the grant *does* wait on the CPU's bus cycle at some phases. The method's control passes on hardware (IWRAM row 227, spread 0) and **fails in mGBA** (spread −1), so mGBA is not a third opinion here. dingbat is flat 226 across every k: it models no deferral at all, which is the mechanism `DMA Prefetch Break` plausibly needs, and it sits 1 cycle below hardware on every undeferred row including the IWRAM control — p50 fixed the H-blank request against absolute stamps, so that residual is more likely the V-blank DMA's. **p54 HDMAMUL run 2026-09-18** settles the rule: with the poll loop on a multiply (four internal cycles, bus idle) hardware is 227 on every k in all six runs with zero variation, where the ROM-load loop defers. Per the pages' own decision table that is "the grant waits for the **bus cycle** specifically" — an internal cycle never delays it, and the load page's non-flat shape is therefore a real bus deferral, not noise. dingbat sits 1 cycle low on all three loops (ROM load, IWRAM, multiply), which points at a constant offset in one of the two DMAs rather than anything phase-dependent; p50 pinned the H-blank request, so look at the V-blank side. Nothing changed: the deferral is the other session's piece (docs/playtest-bugs.md §12). **Prototyped 2026-09-18** (docs/playtest-bugs.md §14): a `Bus.access_end` stamp plus an `etHDMARequest` that re-arms itself there makes hdmasweep vary for the first time while hdmamul stays flat, so the rule is reproducible here — but dingbat then defers on rows hardware leaves at 227, and the base delay still has to move to flag+1 to match hdmamul. Not shipped. The same session proved no open-bus window can close `DMA Prefetch Break` (a 13x13 bounds sweep reaches three values; an offline enumeration over every sub-instruction slot shows the exit counts are dense), and that request delay 9–12 turns the whole suite green with zero collateral — which is the size of the missing deferral, about seven cycles. **Corrected 2026-09-18:** hdmamul does NOT refute that delay — it reports the V-blank DMA's write minus the H-blank DMA's, so it is blind to a common-mode shift (moving both grants together holds it at 226 across the whole range while the suite goes green). p50's absolute V-blank stamp is what refutes it. The 1-cycle deficit under all three loops was the **V-blank DMA's**, as this row guessed: request at flag+1 makes hdmamul and hdmasweep's undeferred rows read 227 exactly, gates unchanged — SHIPPED. **CLOSED 2026-09-18 by p55 LINEGEO and p56 HDMAGEO** (docs/playtest-bugs.md §17): both start one timer, never stop it and report only differences, so the enable latency cancels. linegeo — scanline brackets 1232, H-blank flag brackets 1006, V-blank flag simultaneous with the VCOUNT 160 edge, **hardware = dingbat = mGBA**; the only disagreeing column is the anchored one, by exactly one 7-cycle poll iteration at two phases in seven, which is the whole of hdmastamp's “1006 vs 999” — an artifact, and that lead is withdrawn. hdmageo freezes a second timer with the DMA's own write: over nine runs hardware's grant window relative to the flag is **−11..−5 and dingbat's is the same −11..−5**, two runs byte-identical to the console, mGBA −12..−6. So the grant is NOT the 7–10 cycles late that `DMA Prefetch Break` wants, the deferral is provably inert on that row (flat across caps 4–64), and the row stays red on purpose. Residual: hardware reaches 6 distinct phases inside that window where dingbat reaches 4 |
...
| Thumb open-bus halfword composition (`$+4` vs `$+6` by alignment) | — | **closed 2026-09-18** — tests/roms/payloads/obusprobe.s on the link rig: neither, and not fetch order. The two most recent fetches `$+2` and `$+4` each land in the half of the 32-bit latch **its own address bit 1 selects**, so the same load at 0x030000F0 and 0x030000FA reads `3E0260A8` and `61283E02`. Over a 16-bit bus one fetch fills both halves and the duplicate stands, so the pair applies in BIOS/IWRAM/OAM only — composing everywhere costs 40 mGBA suite I/O rows. **landed** in `read_open_bus_value`; runner and suite row-identical (docs/playtest-bugs.md §11). **Gate checked 2026-09-18** — obusbus.s runs the identical copied block from four memories: EWRAM and VRAM (16-bit) both duplicate `[$+4]`, so the gate is hardware-correct and those 46 suite rows hid nothing; but OAM (32-bit) hands back the aligned **word** holding `$+4`, as ARM code gets, not IWRAM's pair. Bus width is the wrong rule — what decides it is how much of the latch one fetch fills, three cases not two. OAM **landed**, both gates row-identical. BIOS stays with IWRAM, unmeasurable (docs/playtest-bugs.md §12) |
| HLE arithmetic **values** (not costs): Div by zero, `0x80000000 / -1`, negative rounding, both ends of Sqrt, ArcTan2 on the axes and at the origin | swiedge.s over the link rig | **closed 2026-09-18** — 0 of 96 bytes disagree: hardware, dingbat and mGBA identical on all 24 answers, `GetBiosChecksum` = `BAAE187F`. The HLE arithmetic is right against Nintendo's own BIOS, not merely self-consistent. Costs remain open in the row below |
...
| **PSG ch1 shift-0 trigger, off the slow timing.** Modelled (2f1024d3, laws fsfirst/s0trig/s0time/s0long/s0write): an armed f = 0x400 shift-0 trigger dies; the first check after a master-on uses a slow timing anchored to the master-on's 4 MHz edge, ended by a sweep calculation or a 512 Hz step while halted. Open: once off it, s0trig's bit-14 cells stop on the first 4 MHz edge 3+ cycles on ("C2", no survivor) while s0path's halted cells capture on a 4 MHz edge and miss a write one cycle before it ("C3", one survivor in 4). s0mode: the choice moves with the master-on's phase (j), the trigger's distance from the wake (u) and a previous ch1 note (h = 1 gives C2) — several inputs, no rule yet. dingbat takes C2 everywhere; no game in an 11-game census reaches it | payloads/s0path.s, s0mode.s; answers in `tools/hwlink/open-agb.json` (psg_shift0) | **run 2026-09-25**, unexplained. Also there: fsgap (the forced length clock after a master-on, bounded 4 < W ≤ 8; past it the 512 Hz tap decides run to run) and s0time's step-synced half (sync too coarse). dbsuite apu reds left: psg-ch1-after-ch2-lives (a run-to-run lottery on the console), sweeptrig-shift0-400-after-rows-d33 |
| **Sound-FIFO refills at timer periods 1–3** (hades/dma-latch: the console shows the DMA's word at 5 NOPs, dingbat at 8). At k = 1..3 the console runs refills in groups (26, then +18 every 14 NOPs at k = 1) and holds them off longer; dingbat does single 10-cycle bursts one request apart. A parameter grid over request/grant/chain rules found nothing (best 93/287 cells wrong) | payloads/fifoffff.s (header has the per-k tables); answers in `tools/hwlink/open-agb.json` (fifo) | **run 2026-09-25**, no model. The fifodma "spikes" in the same area were a previous cell's TM0 left at 0xFFFF — landed as `TIMER_START_OVERFLOW` |
| **dbsuite contention cases** bus/contend-pram-mode0, contend-vram-mode0, contend-vram-mode2-locked-out, dma/dmatime-burst-vram-mode3: their expected values are gbaedge p41/p46 photographs, and nothing in dbsuite fixes the dot their reads land on, so the result depends on every earlier case's length. The laws (contmap, c2seq, dmavram) predict the photographed values at one phase | branch `worktree-agent-aa8ff0fb965ceea0e`: a V-count park per page in the cartridge build only (72008a06) and the run_case progress-draw fix (e4999c9c) | **held** for a decision; a flash-cart run of the cartridge build would confirm the parked phases. dmavram (law) confirms the DMA side on the console |
| Rig hazard: the round-1 FIFO probe (job scratch `fifomap.s`, not the repo's `payloads/fifomap.s`) and `payloads/fifoffff.s` at k = 6..20 each wedged the console mid-session (power cycle needed); dingbat runs both. Suspects: DMA1 left armed across the exit, the V-blank DMA0 guard | — | not understood; don't resend those cells without a change to the exit path |
...
| ~~gbaedge page: open-bus reads by the DMA itself and by instructions 1–3 after a burst~~ | done over the link instead | **run 2026-09-18** — obusprobe.s: hardware holds the word for exactly one instruction (read 0 after the enable store = opcode, 1 = the DMA word, 2 = opcode). dingbat holds it one too long, mGBA releases it one too early. NOT changed: three games depend on that comment's window and this probes one shape only (immediate DMA3 to EWRAM from ARM). `fetch_start` after a burst is the suspect, not the window (docs/playtest-bugs.md §11) |

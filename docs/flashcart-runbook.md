...
| gba-suite | the very `mgba-suite.gba` we test against (`/tmp/dingbat-test-roms/mgba-suite.gba`, or `deploy/suite.gba` from the suite checkout) | **unrun, and the highest-value run on this list.** Photograph the Misc. edge case page. Nobody has ever read this build's `DMA Prefetch Break` off silicon; the expected `0x10002A94` is a hardcoded constant of unknown provenance (docs/playtest-bugs.md section 20) | docs/playtest-bugs.md section 20 |
| gba-prefetchdma | `tests/roms/prefetchdma.gba` | **unrun, and now the run that decides the row.** Part A's first cell is the whole experiment: the period of the seven-instruction Thumb loop in ROM at 4/2 waits, prefetcher off. dingbat says **36 cycles**, and at 36 the suite's expected `0x10002A94` is *unreachable from any entry phase*; only 31, 34, 37 or 38 can produce it (docs/playtest-bugs.md section 21). 36 means the constant is not a property of this loop; 37 or 38 means a real ROM-fetch timing bug. Then the rest: the loop period, what an H-blank DMA costs a ROM-fetched loop, the post-DMA open-bus window from ROM code, and a replica of the suite's own loop. Attach the link adapter and run `python3 tools/hwlink/gblink.py report`; no photograph needed, and the block is in SRAM as a fallback. Decode with `python3 tests/roms/prefetchdma.py decode FILE` | tests/roms/prefetchdma.s header |

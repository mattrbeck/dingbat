...
| gba-suite | the very `mgba-suite.gba` we test against (`~/Documents/emu/gba/flashcart-tests/mgba-suite.gba`, SHA1-checked against the runner's pin) | **unrun, and the one run left that matters for the row.** Photograph the Misc. edge case page. Every mechanism `DMA Prefetch Break` depends on has since been measured on the AGB SP without a cartridge (docs/playtest-bugs.md section 22), and with those the prediction for silicon is **`0x10002540`**, not the suite's hardcoded `0x10002A94`. `0x10002540`: the expectation is wrong and the row is closed as one. `0x10002A94`: about fifteen cycles are hiding in the suite's own interrupt dispatcher | docs/playtest-bugs.md sections 20, 22 |

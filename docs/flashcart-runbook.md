...
| gba-suite | three builds in `~/Documents/emu/gba/flashcart-tests/`: `mgba-suite-upstream-official.gba` (upstream's buildbot binary, menu-driven), `mgba-suite-irqreset.gba` (our fork with libgba's interrupt table reset before each suite), `mgba-suite.gba` (the fork as currently released) | **unrun.** Photograph the Misc. edge case page of each. `DMA Prefetch Break` was red because the fork's back-to-back run leaves SIO timing's `TIMER1` handler in libgba's table, and every V-blank dispatch steps over it: 11 cycles, eleven scanlines of loop (docs/playtest-bugs.md section 23). Expect `0x10002A94` from the first two and `0x10002540` from the third. On the official build, also run SIO timing and *then* Misc: the value should change, which is an upstream bug in the test | docs/playtest-bugs.md sections 22, 23 |

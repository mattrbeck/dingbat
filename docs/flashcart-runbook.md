...
| gba-pages3 | `tests/roms/gbaedge.gba` page 50 (HDMAPHASE, hex) | **unrun** — one photograph settles whether an H-blank DMA is granted on the cycle-960 edge or on the latched DISPSTAT flag. It is the last page in the viewer, so page right past UNDMODE (or hold SELECT at power-on to skip UNDMODE and reach it safely) | tests/roms/README-probes-gba.md "page v9" |

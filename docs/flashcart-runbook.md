...
| gba-suite | three builds in `~/Documents/emu/gba/flashcart-tests/`: `mgba-suite-upstream-official.gba` (upstream's buildbot binary, menu-driven), `mgba-suite-irqreset.gba` (our fork with libgba's interrupt table reset before each suite), `mgba-suite.gba` (the fork as currently released) | **unrun, and the last word on `DMA Prefetch Break`.** Photograph the Misc. edge case page of each; on the official build do it twice, from a fresh boot and again after running SIO timing. dingbat (HLE and real BIOS alike) predicts `0x10002B1C` from a fresh table and `0x10002540` with SIO timing's handler left in it; the suite's constant `0x10002A94` is what mGBA computes and nothing else (docs/playtest-bugs.md sections 23, 24). `0x10002B1C`: the constant is mGBA's, send the correction upstream. `0x10002A94`: one cycle is still hiding, most likely in a mechanism section 24 lists as unmodelled | docs/playtest-bugs.md sections 22-24 |

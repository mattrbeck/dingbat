...
- [`LY_BLIND_SCOPE`](#ly_blind_scope)
- [`STAT_IRQ_LEAD`](#stat_irq_lead)
- [`STAT_LYC_LEAD`](#stat_lyc_lead)
- [`STAT_IRQ_SPLIT` / `STAT_DOMAIN_LEAD`](#stat_irq_split--stat_domain_lead)
- [`STAT_READ_SAMPLE`](#stat_read_sample)
- [`STAT_READ_SAMPLE_DS_ADD`](#stat_read_sample_ds_add)
- [`STAT_M0_FIELD_TAIL`](#stat_m0_field_tail)
- [`STAT_M0_FIELD_TAIL_CGB`](#stat_m0_field_tail_cgb)
- [`STAT_M0_TAIL_MAX_MC`](#stat_m0_tail_max_mc)
- [`STAT_M0_FIELD_TAIL_ABSORB`](#stat_m0_field_tail_absorb)
- [`STAT_MODE3_LAG`](#stat_mode3_lag)
- [`STAT_MODE3_LAG_CGB`](#stat_mode3_lag_cgb)
- [`STAT_M0_TAIL_ANY`](#stat_m0_tail_any)
- [`STAT_MODE_LAG_ANY` / `STAT_NO_HOLD`](#stat_mode_lag_any--stat_no_hold)
- [`GDMA_SETUP_MCYCLES`](#gdma_setup_mcycles)
- [`SERIAL_TAP_DMG` / `SERIAL_TAP_CGB`](#serial_tap_dmg--serial_tap_cgb)
- [`SERIAL_START_ARM` (the residual `start_wait_*` cluster)](#serial_start_arm-the-residual-start_wait_-cluster)
- [`CGB_HALT_EXIT_MCYCLES`](#cgb_halt_exit_mcycles)
- [`CGB_HALT_LEAD_LYC_ONLY`](#cgb_halt_lead_lyc_only)
- [`CGB_HALT_LEAD_SKIP_LYC0`](#cgb_halt_lead_skip_lyc0)
- [`CGB_HALT_PPU_LEAD`](#cgb_halt_ppu_lead)
- [`CGB_OAM_DMA_START_T`](#cgb_oam_dma_start_t)
- [`GB_POWERUP_WRAM_PATTERN`](#gb_powerup_wram_pattern)
- [`HDMA_STEAL_DELAY_M`](#hdma_steal_delay_m)
- [`HDMA_BLOCK_OVERHEAD_BUS`](#hdma_block_overhead_bus)
- [`HDMA_BLOCK_OVERHEAD_DOTS`](#hdma_block_overhead_dots)
- [`HDMA_VISIBLE_DOTS`](#hdma_visible_dots)
- [`CGB_HALT_PPU_LEAD_DOTS`](#cgb_halt_ppu_lead_dots)
- [CGB per-register PPU write latency](#cgb-per-register-ppu-write-latency)
- [`CGB_OBJ_SCAN_LEAD`](#cgb_obj_scan_lead)
- [`CGB_MAP_LATENCY`](#cgb_map_latency)
- [`CGB_TDSEL_LATENCY`](#cgb_tdsel_latency)
- [`CGB_TDSEL_GLITCH`](#cgb_tdsel_glitch)
- [`CGB_TDSEL_IDX_DOTS`](#cgb_tdsel_idx_dots)
- [`CGB_TDSEL_ANY` / `CGB_MAP_ANY`](#cgb_tdsel_any--cgb_map_any)
- [`CGB_WY_LATCH_LATENCY` / `WIN_EN_ABORT`](#cgb_wy_latch_latency--win_en_abort)
- [`WIN_EN_HOLD`](#win_en_hold)
- [`CGB_WIN_EN_HOLD`](#cgb_win_en_hold)
- [`WIN_EN_HOLD_BACK`](#win_en_hold_back)
- [`WIN_EN_HOLD_ZERO`](#win_en_hold_zero)
- [`WIN_LINE_START_WX`](#win_line_start_wx)
- [`WIN_HEAD_ABSORB`](#win_head_absorb)
- [`WIN_WX0_PHASE`](#win_wx0_phase)
- [`WIN_LINE_START_LATCH`](#win_line_start_latch)
- [`WIN_START_PRE_PIXEL`](#win_start_pre_pixel)
- [`WIN_PRE_PX_PHASE`](#win_pre_px_phase)
- [`WIN_RESTART_COUNTER` / `CGB_WIN_RESTART_COUNTER`](#win_restart_counter--cgb_win_restart_counter)
- [`WIN_TAIL_FETCH`](#win_tail_fetch)
- [`DMG_WIN_START_LAST_PX`](#dmg_win_start_last_px)
- [`DMG_WIN_LAST_PX_CARRY`](#dmg_win_last_px_carry)
- [`WIN_CARRY_TILE`](#win_carry_tile)
- [`WIN_CARRY_REACT_LINES`](#win_carry_react_lines)
- [`CGB_WIN_TAIL_LAST`](#cgb_win_tail_last)
- [`OBJ_BG_RUN`](#obj_bg_run)
- [`M3_THROWAWAY_DOTS`](#m3_throwaway_dots)
- [`OBJ_ABORT`](#obj_abort)
- [`CGB_OBJ_ABORT`](#cgb_obj_abort)
- [`OBJ_ABORT_LEAD`](#obj_abort_lead)
- [`OBJ_ABORT_FLAG_HOLD`](#obj_abort_flag_hold)
- [`MIXER_PRIORITY_BACK`](#mixer_priority_back)
- [`BG_EN_AT_MIX`](#bg_en_at_mix)
- [`MIXER_PALETTE_BACK`](#mixer_palette_back)
- [`MIXER_PALETTE_OR`](#mixer_palette_or)
- [`MIXER_DOT_LAG`](#mixer_dot_lag)
- [`MIXER_TAIL_HBLANK`](#mixer_tail_hblank)
- [`NO_LCDC2_FLIP`](#no_lcdc2_flip)
- [`OBJ_FIX_OFF`](#obj_fix_off)
- [`NO_TDSEL_CHANGE`](#no_tdsel_change)
- [`NO_MAP_CHANGE`](#no_map_change)
- [`TDSEL_ADDR_OFF`](#tdsel_addr_off)
- [`TDSEL_ADDR_BANK`](#tdsel_addr_bank)
- [`TDSEL_IDX_SHIFT`](#tdsel_idx_shift)
- [`MIXER_TAIL_DOTS`](#mixer_tail_dots)
- [`MIXER_HEAD_LINGER`](#mixer_head_linger)
- [`MIX_HOLD`](#mix_hold)
- [`CGB_MIXER_LATENCY`](#cgb_mixer_latency)
- [`CGB_LCDC_MIXER_LATENCY`](#cgb_lcdc_mixer_latency)
- [`CGB_LATENCY_CAP`](#cgb_latency_cap)
- [`LCD_ON_LINE0_TRIM` / `LCD_ON_LINE1_TRIM`](#lcd_on_line0_trim--lcd_on_line1_trim)
- [`SCX_FINE_LATCH_LIVE`](#scx_fine_latch_live)
- [`SCX_FINE_LATCH_WRAP`](#scx_fine_latch_wrap)
- [`SCX_STORE_STALL_DOTS`](#scx_store_stall_dots)
- [`fifo_obj_abort` (OBJ_ABORT, OBJ_ABORT_LEAD, OBJ_ABORT_FLAG_HOLD, CGB_OBJ_ABORT)](#fifo_obj_abort-obj_abort-obj_abort_lead-obj_abort_flag_hold-cgb_obj_abort)
- [The mixer tail (MIXER_PRIORITY_BACK, MIXER_PALETTE_BACK, MIXER_TAIL_DOTS, MIXER_TAIL_HBLANK, MIXER_DOT_LAG)](#the-mixer-tail-mixer_priority_back-mixer_palette_back-mixer_tail_dots-mixer_tail_hblank-mixer_dot_lag)
- [`obj_yields_to_window` (the window/object tie)](#obj_yields_to_window-the-windowobject-tie)
- [`M3_PIPE_MCYCLES` / `M3_PIPE_DELAY`](#m3_pipe_mcycles--m3_pipe_delay)
- [`tick_sprite_fetcher` / `OBJ_BG_RUN` (which dots the BG fetcher may run on)](#tick_sprite_fetcher--obj_bg_run-which-dots-the-bg-fetcher-may-run-on)
- [`OBJ_PLANE_GAP` / `OBJ_PLANE1_LAG` / `OBJ_PLANE1_HEAD` / `CGB_OBJ_SIZE_LATENCY`](#obj_plane_gap--obj_plane1_lag--obj_plane1_head--cgb_obj_size_latency)
- [`OBJ_FETCH_DOTS` / `OBJ_WAIT_SUB` (the OBJ penalty)](#obj_fetch_dots--obj_wait_sub-the-obj-penalty)
- [`fifo_head_window` (WIN_LINE_START_LATCH, WIN_HEAD_ABSORB)](#fifo_head_window-win_line_start_latch-win_head_absorb)
- [A window-start line still pays SCX & 7 (WIN_WX0_PHASE)](#a-window-start-line-still-pays-scx--7-win_wx0_phase)
- [`SCX_FINE_LATCH_WRAP` / `SCX_STORE_STALL_DOTS`](#scx_fine_latch_wrap--scx_store_stall_dots)
- [`SCX_FINE_BORROW` / `SCX_FINE_BORROW_DMG_LEAD` / `SCX_FINE_LATCH_LIVE`](#scx_fine_borrow--scx_fine_borrow_dmg_lead--scx_fine_latch_live)
- [`OAM_SCAN_DMA_LOCK` / `OBJ_SCAN_DOT_ADJ` / `CGB_OBJ_SCAN_LEAD`](#oam_scan_dma_lock--obj_scan_dot_adj--cgb_obj_scan_lead)
- [`ly_advance_open` / `ly_advance_close` / `LY_BLIND_SCOPE` (the comparator blind window)](#ly_advance_open--ly_advance_close--ly_blind_scope-the-comparator-blind-window)
- [`LY153_SNAP_DOT` / `LYC_SETTLE_DOTS`](#ly153_snap_dot--lyc_settle_dots)
- [`STAT_M2_LEAD` / `STAT_M2_EARLY_LY0` / `STAT_M2_LEAD_CGB`](#stat_m2_lead--stat_m2_early_ly0--stat_m2_lead_cgb)
- [`STAT_READ_SAMPLE` / `STAT_READ_SAMPLE_DS_ADD`](#stat_read_sample--stat_read_sample_ds_add)
- [`HALT_IF_SAMPLE_T` (where a halted CPU latches IF)](#halt_if_sample_t-where-a-halted-cpu-latches-if)
- [`SPEED_SWITCH_STALL_T` / `SPEED_SWITCH_STALL_CPU` / `SPEED_SWITCH_STALL_RUNS_CPU_CLOCK`](#speed_switch_stall_t--speed_switch_stall_cpu--speed_switch_stall_runs_cpu_clock)
- [`SPEED_SWITCH_PPU_EXTRA_DOTS` / `SPEED_SWITCH_PPU_EXTRA_DOTS_SINGLE`](#speed_switch_ppu_extra_dots--speed_switch_ppu_extra_dots_single)
...
## `ly_advance_open` / `ly_advance_close` / `LY_BLIND_SCOPE` (the comparator blind window)
...
## `LY153_SNAP_DOT` / `LYC_SETTLE_DOTS`
...
## `STAT_M2_LEAD` / `STAT_M2_EARLY_LY0` / `STAT_M2_LEAD_CGB`
...
## `STAT_READ_SAMPLE` / `STAT_READ_SAMPLE_DS_ADD`
...
## `HALT_IF_SAMPLE_T` (where a halted CPU latches IF)
...
## `SPEED_SWITCH_STALL_T` / `SPEED_SWITCH_STALL_CPU` / `SPEED_SWITCH_STALL_RUNS_CPU_CLOCK`
...
## `SPEED_SWITCH_PPU_EXTRA_DOTS` / `SPEED_SWITCH_PPU_EXTRA_DOTS_SINGLE`

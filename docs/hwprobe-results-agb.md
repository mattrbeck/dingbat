...
## Session 4 — the v6 isolation pages (37-page build @ aaaa0eb)

`ALL B473`.  Raw: `tests/roms/expected/agb-sp-4.txt`.  These nine pages
were designed to discriminate exactly what sessions 2-3 could not (see
the 2026-08-10 amendments above).  Every ambiguity is now settled; five
pages match dingbat byte-perfect, one matches in every behavior bucket,
and three expose real divergences:

| page | verdict |
|---|---|
| LDMUSER2 | **MATCH (byte-perfect, CRC B520)** — with CPSR flags (Z\|C) fully disjoint from the SPSR value (N\|V), the SPSR read in the ldm^ shadow returns **90000092 = SPSR UNCHANGED**, at both one and two instructions' distance.  The OR-with-CPSR theory (would read F0000092) and the returns-CPSR theory (60000092) are now both properly falsified — session 2's non-discriminating row is superseded |
| PCWB2 | four of five rows match: `str [r15],#8` -> PC=base+8, `ldr [r15],#8` -> PC=base+12 + load suppressed, `str [r15],#-4` -> PC=base-4, pre-indexed `str [r15,#4]!` -> PC=base+4 — **"PC := the writeback address (+4 for ldr)" is the confirmed functional form** (the fixed-base+4/base+8 alternative is dead).  DIVERGENCE on row 5: **`stmia r15!, {r1}` performs NO writeback on hardware** (breadcrumb 1F, like ldm); dingbat had generalized it to a base+8 branch (18) — **fixed**: the block-store path now suppresses r15 writeback like the load path, and the page reads byte-perfect (CRC A9AA).  Stored-value forensic words identical on both |
| DMABYTE2 | **MATCH (byte-perfect, CRC 1A71)** — the CNT_H byte-write anomaly is **bit7-granular and present on ALL FOUR channels**: hi-byte strb 0x80 mirrors into the low byte on DMA0/1/2/3 alike (0080); hi-byte 0xC0 reads back **4080** (only bit7 mirrors down — the whole-value-mirror model is dead); lo-byte 0xC0 reads back **0040** (bit7 dropped, other bits store — the whole-write-dropped model is dead); hi-byte 0x40 stores normally (4000).  Dingbat's bit7-granular generalization was exactly right |
| SWEEP2 | model-bucket match on all four rows (poll counts jitter as designed: hw 35CD/0A75/1138/0002 vs dingbat 3532/09D8/109D/0000).  freq-512/shift-1 dies at **tick 3** -> the trigger checks do NOT write back and the tick path DOES run a recalculated second check; freq-2018/shift-7 (second trigger check = exactly 2048) **survives the trigger** -> the strictly->2048 boundary is re-anchored, and dies at **tick 1** -> the tick-path second check fails at >=2048; freq-940/shift-1 survives the trigger -> the trigger's second check reuses the OLD offset (third anchor); freq-2033/shift-7 (first calc = exactly 2048) dies at the trigger.  **Every previously single-anchored point in the sweep model is now multiply anchored; dingbat's model is confirmed in full** |
| IRQWIN3 | **MATCH (byte-perfect, CRC C443)** — the dispatch window is cycle-based with dingbat's exact constant: 2 muls / 2 ROM-ldrs / 3 mixed-sled instructions (08/08/0C bytes), and the re-designed ack-race sweep now straddles the ack instant and matches bit-for-bit (03 03) |
| IRQLAT2 | structural match, timing offset: one-word reload-0 arming DELIVERS (the session-2 TM2 retraction is confirmed on silicon), one-word vs two-write arming are byte-equal (Δ 0x143 both — arming shape irrelevant), and the 0xFFF0 row shows entry < trigger (overflow lands inside the arming window) on both machines.  DIVERGENCE: hardware's handler-entry latency is **+6 cycles vs dingbat in every row** (Δ 0x39 vs 0x33; 0x143 vs 0x13D) — same direction as the session-1 IRQ_SYNC_DELAY anchors, now with a clean minimal shape to tune against — **fixed**: the +6 (and the +7-per-round-trip trigger drift) is a gamepak-execution-context cost at the vector entry/return, not a sync-delay change; the page now reads byte-perfect (CRC 0EE1) with the mGBA suite unmoved |
| IOBYTE2 | behavioral **MATCH** — with the falsifiable 0x38 payload (real IRQ-enable bits), **DISPSTAT's low byte STORES byte writes** (0039), the byte-clear stores too (0001), and the LYC hi-byte strb lands (3801).  Session 3's "ignores byte writes entirely" is now formally dead; the 0x44 row really was just bits 6-7 not existing, which is what dingbat models.  (The +8 info field differs — 0003 vs 0001 — but it is the *saved boot DISPSTAT*, a documented phase-dependent informational cell, not behavior) |
| THUMBPC3 | (b) System-mode `cmp pc` = **C000001F on both** — with no real SPSR the restore is a no-op (CPSR flags survive untouched); clean on both machines.  (a) DIVERGENCE: at a halfword-aligned address with a T-clearing SPSR, hardware resumes ARM execution **past BOTH overlay words** — the strmi at A-2's word and the add at A+6 never execute (scratch 0, r6 0), i.e. resume is at A+10 (the `bx` escape), while dingbat resumes at (A+2)&~3 and executes the A+6 word (r6=2).  Consistent with the MSRTBIT-style pipeline pattern (discard the prefetched slots, refill from the aligned fetch address +8) rather than a plain PC&~3.  The word-aligned case (session 3) is unaffected.  Needs a small follow-up row to pin A+10 vs A+14 before modeling — **built as the v7 slot-35 row (c)** (resume ladder W+8/W+12/W+16, r4 captured; see docs/hwprobe-questions.md) |
| MSRTBIT2 | **MATCH (byte-perfect, CRC E35E)** — the MSR T-set quirk (resume A+8, skip A+10) is **form-independent** (immediate `msr CPSR_c, #0x3F` behaves identically) and **composes with a mode switch**: setting T and switching IRQ->SYS in one write shows the same breadcrumb (04) and the mode switch lands (recovery mode byte 1F).  The remaining scope caveats on this quirk are closed |

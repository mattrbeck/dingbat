...
| PCWB2 | four of five rows match: `str [r15],#8` -> PC=base+8, `ldr [r15],#8` -> PC=base+12 + load suppressed, `str [r15],#-4` -> PC=base-4, pre-indexed `str [r15,#4]!` -> PC=base+4 — **"PC := the writeback address (+4 for ldr)" is the confirmed functional form** (the fixed-base+4/base+8 alternative is dead).  DIVERGENCE on row 5: **`stmia r15!, {r1}` performs NO writeback on hardware** (breadcrumb 1F, like ldm); dingbat had generalized it to a base+8 branch (18).  Stored-value forensic words identical on both |

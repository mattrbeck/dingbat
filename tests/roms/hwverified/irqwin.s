...
@ before dispatch (iw_sled converts it to a byte offset).  A second IME
@ run uses a sled of EWRAM loads instead: multi-cycle instructions
@ disambiguate instruction-counted from cycle-counted sampling.
@ Finally, 16 back-to-back IF acks race a TM2 that overflows every 16
@ cycles, counting how many acks saw the flag still (or again) set.
@
...
@ for the msr (0008), and with the EWRAM-load sled after TWO loads
@ (0008) — the window is cycle-based, not instruction-based.  The
@ ack-race count was 8 of 16 on hardware; it is a bus-phase-sensitive
@ count, so the slot stores it as a word at +12 and it is RANGE-checked
@ (6..10) rather than exact-checked — a false red on real hardware
@ would be worse than a loose band.
@
@ PROVENANCE: verified on GBA SP AGS-001 (sessions 2+3, gbaedge pages
@ 20 IRQWIN and 26 IRQWIN2); see docs/hwprobe-results-agb.md.
    .arm
    .text
    .global _start
_start:

## ImGui "Cheats" window. Owns no core state: it mutates the `CheatEngine`
## the app hands it and calls `on_change` after any edit so the app can
## re-apply ROM patches and persist.

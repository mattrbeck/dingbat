...
#
# Transport-agnostic (like netcore): the transport calls `tick(local_bits)` once
...
proc new_rollback_session*(link: Link; local: int; max_ahead = 12): RollbackSession =
  ...
  ## peer's buttons drive. `max_ahead` bounds prediction depth — pick it above the
  ...
  result = RollbackSession(
    ...
    ring: newSeq[LinkSnapshot](cap), ring_frame: newSeq[int](cap), cap: cap)
...
proc tick*(sess: RollbackSession; local_bits: uint16): RollbackStatus =

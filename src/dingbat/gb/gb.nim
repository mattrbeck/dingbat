...
# Covers tile data $8000-$819F: the blank tile $00, the decompressed Nintendo
# logo (tiles $01-$18), and the ® trademark tile ($19). Several Mealybug PPU
# tests (e.g. m3_scx_low_3_bits) place the ® tile on screen without loading it
# themselves, relying on the boot ROM having left it in VRAM — so tiles $18/$19
# must be present, not just $01-$17. Bytes verified against the DMG boot ROM's
# logo-decompression of the cartridge header logo.
const POST_BOOT_VRAM*: array[416, uint8] = [
  ...
  # tile $18 — final Nintendo-logo tile
  0x0F, 0x00, 0x0F, 0x00, 0x0F, 0x00, 0x0F, 0x00, 0x0F, 0x00, 0x0F, 0x00, 0xFC, 0x00, 0xFC, 0x00,
  # tile $19 — ® trademark symbol (boot ROM TrademarkLogo, low plane only)
